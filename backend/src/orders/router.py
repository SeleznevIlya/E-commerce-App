...
async def create_order(promocode: str = None, user: UserModel = Depends(get_current_user)):
    return await OrderService.create_new_order(user_id=user.id, promocode=promocode)
...
@order_router.get("/me")
async def get_my_orders(user: UserModel = Depends(get_current_user)):
    pass

@order_router.get("/{user_id}")
async def get_orders_by_user(user_id: str, user: UserModel = Depends(get_current_superuser)):
    pass
