...
class OrderRepository(BaseRepository[OrderModel, OrderCreateDB, OrderUpdate]):
    ...
    @classmethod
    async def find_one_with_association_model(cls, session: AsyncSession, association_model, **filter_by) -> Optional[OrderModel]:
        stmt = (
            select(cls.model).
            filter_by(**filter_by).
    ...
    @classmethod
    async def find_all_with_association_model(cls, 
